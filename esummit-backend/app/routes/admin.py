...
def index():
    ...
    try:
        ...
        pitch_count = PitchRegistration.query.count()
        active_registrations = EventRegistration.query.filter_by(status='confirmed').count()

        # Get recent registrations
        recent_registrations = EventRegistration.query.order_by(EventRegistration.registration_date.desc()).limit(5).all()

        # Get recent pitch registrations
        recent_pitches = PitchRegistration.query.order_by(PitchRegistration.registration_date.desc()).limit(5).all()
...
@admin_required
def pitches():
    """List all pitch registrations"""
    page = request.args.get('page', 1, type=int)
    per_page = 10  # Number of registrations per page

    # Get filtered pitch registrations
    pitches_query = PitchRegistration.query.order_by(PitchRegistration.registration_date.desc())
    ...
    if event_id:
        pitches_query = pitches_query.filter_by(event_id=event_id)
    if status:
        pitches_query = pitches_query.filter_by(status=status)
    if search:
        pitches_query = pitches_query.join(Team).filter(
            Team.name.ilike(f'%{search}%')
        )

    # Get paginated results
    pagination = pitches_query.paginate(page=page, per_page=per_page, error_out=False)
    pitches = pagination.items

    # Get all pitching events for the filter dropdown
    events = Event.query.filter_by(event_type='pitching').order_by(Event.start_date.desc()).all()
...
@admin_bp.route('/pitches/<int:pitch_id>')
@admin_required
def view_pitch(pitch_id):
    """View pitch registration details"""
    pitch = PitchRegistration.query.get_or_404(pitch_id)
    return render_template('admin/view_pitch.html', pitch=pitch)

@admin_bp.route('/pitches/<int:pitch_id>/approve', methods=['POST'])
@admin_required
def approve_pitch(pitch_id):
    ...
    db.session.commit()
    flash(f'Pitch "{pitch.pitch_title}" has been approved!', 'success')
    return redirect(url_for('admin.view_pitch', pitch_id=pitch.id))

@admin_bp.route('/pitches/<int:pitch_id>/reject', methods=['POST'])
@admin_required
def reject_pitch(pitch_id):
    """Reject a pitch registration"""
    pitch = PitchRegistration.query.get_or_404(pitch_id)
    reason = request.form.get('reason', '')
    pitch.status = 'rejected'
    pitch.rejection_reason = reason
    db.session.commit()
    flash(f'Pitch "{pitch.pitch_title}" has been rejected.', 'success')
    return redirect(url_for('admin.view_pitch', pitch_id=pitch.id))
...
@admin_bp.route('/export/pitches', methods=['POST'])
@admin_required
def export_pitches():
    """Export pitch registrations data"""
    format = request.form.get('format', 'csv')
    fields = request.form.getlist('fields')

    # Get all pitch registrations
    pitches = PitchRegistration.query.order_by(PitchRegistration.registration_date.desc()).all()
    ...
    flash('Export started! You will receive an email when it\'s ready.', 'info')
    return redirect(url_for('admin.pitches'))
